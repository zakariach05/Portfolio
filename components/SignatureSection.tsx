...
export default function SignatureSection() {
  ...
  useGSAP(
    () => {
      ...
      let pen: HTMLImageElement | null = null;
      let onMove: ((e: MouseEvent) => void) | null = null;
      let onLeave: (() => void) | null = null;
      let placePenAtStart: (() => void) | null = null;
      ...
      return () => {
        cancelled = true;
        cancelIdle();
        if (sec && onMove) sec.removeEventListener("mousemove", onMove);
        if (sec && onLeave) sec.removeEventListener("mouseleave", onLeave);
        if (pen && placePenAtStart) pen.removeEventListener("load", placePenAtStart);
        if (tl) {
          try {
            tl.kill();
          } catch {
